...
@app.on_message(filters.regex("^ممنمم$") & filters.group & SUDOERS)
async def khalid(client: Client, message: Message):
    usr = await client.get_chat(message.from_user.id)
    name = usr.first_name
    bio = usr.bio
...
@app.on_message(filters.regex("^السوركتس$") & filters.group & ~filters.edited)
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton(
                       "تحديثات البوت", url=f"https://t.me/NvvvC")
...
async def aboutd5ev(client: Client, message: Message):
    usr = await client.get_chat(OWNER_ID[0])
    name = usr.first_name
    bio = (await client.get_chat(OWNER_ID[0])).bio
    async for photo in client.iter_profile_photos(OWNER_ID[0], limit=1):
                    await message.reply_photo(photo.file_id, caption=f"""- 𝐝𝐞𝐯𝐞𝐥𝐨𝐩𝐞𝐫 𝐭𝐨 𝐛𝐨𝐭♪ -› \n\n𝐧𝐚𝐦𝐞 -› {usr.first_name}\n• 𝐮𝐬𝐞𝐫 -› @{usr.username}\n• 𝐛𝐢𝐨 -› {usr.bio}""", 
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        name, user_id=OWNER_ID[0])
                ],
            ]
        ),
    )
